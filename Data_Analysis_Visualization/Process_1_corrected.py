...
files = glob.glob(f'{folderdate}/Lubrecht*.h5')
    ...
    try:
        ...
        for aqnum in range(0, len(f.keys()) - 1):
            try:
                ...
                Sun_Position_Altitude = 90 - Sun_Position_Altitude
                    
                if aqnum == 0: 
                    I = uv_data['I_corrected'][:]
                    
                    sza0 = 90 - aq.attrs['Sun Position Altitude']
                    ...
                    # plt.show()
                    f['Measurement_Metadata'].attrs['Center Pixel Corrected (x,y)'] = np.array([x_center,y_center])
                    #Calculate geometry 
                    ...
                    if 'view_az_corrected' not in uv_data:
                        uv_data.create_dataset('view_az_corrected', data = np.degrees(view_az))
                        uv_data.create_dataset('view_zen_corrected', data = np.degrees(view_zen))
                        uv_data.create_dataset('sun_az_corrected', data = np.degrees(sun_az))
                        uv_data.create_dataset('sun_zen_corrected', data = np.degrees(sun_zen))
                    elif 'view_az_corrected' in uv_data:
                        print('here')
                        del uv_data['view_az_corrected']
                        del uv_data['view_zen_corrected']
                        del uv_data['sun_az_corrected']
                        del uv_data['sun_zen_corrected']
                        uv_data.create_dataset('view_az_corrected', data = np.degrees(view_az))
                        uv_data.create_dataset('view_zen_corrected', data = np.degrees(view_zen))
                        uv_data.create_dataset('sun_az_corrected', data = np.degrees(sun_az))
                        uv_data.create_dataset('sun_zen_corrected', data = np.degrees(sun_zen))
        ...
        #print('done')
        f['Measurement_Metadata'].attrs['Processed Level'] = 'Level 1 Corrected'
        f.close()
